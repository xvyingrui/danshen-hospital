...
###5、心血管资源门诊费用
#### 第2年的随访，通过控制第一年的心血管发生次数、住院天数、门诊费用和住院费用以及患者的基线特征（性别、保险类别、年龄）研究是否使用丹参对第2年心血管资源门诊费用的影响。

```{r}
#心血管资源门诊费用

zs.hs.hz1=cb.zs.hs.hz1[,c("心血管资源门诊费用","性别","保险类别","age","是否使用丹参",'一年心血管次数.x','住院天数.x','门诊费用.x','住院费用.x')]

write.table(zs.hs.hz1,file = 'glm3.csv',sep = ',',row.names = F)


#基本情况描述
ggplot(zs.hs.hz1, aes(心血管资源门诊费用, colour = 是否使用丹参)) + geom_density()


with(zs.hs.hz1, tapply(心血管资源门诊费用, 是否使用丹参, function(x) {
    sprintf("M (SD) = %1.2f (%1.2f)", mean(x), sd(x))
}))


#拟合gammar分布回归
zs.hs.hz1$心血管资源门诊费用1=zs.hs.hz1$心血管资源门诊费用+1
m1<-glm(心血管资源门诊费用1~.,data=zs.hs.hz1[,-1],family=Gamma(link = 'log'))
summary(m1)
...
###6、心血管资源住院费用
#### 第2年的随访，通过控制第一年的心血管发生次数、住院天数、门诊费用和住院费用以及患者的基线特征（性别、保险类别、年龄）研究是否使用丹参对第2年心血管资源住院费用的影响。


```{r}
#心血管资源住院费用

zs.hs.hz1=cb.zs.hs.hz1[,c("心血管资源住院费用","性别","保险类别","age","是否使用丹参",'一年心血管次数.x','住院天数.x','门诊费用.x','住院费用.x')]

write.table(zs.hs.hz1,file = 'glm2.csv',sep = ',',row.names = F)


#基本情况描述
ggplot(zs.hs.hz1, aes(心血管资源住院费用, colour = 是否使用丹参)) + geom_density()


with(zs.hs.hz1, tapply(心血管资源住院费用, 是否使用丹参, function(x) {
    sprintf("M (SD) = %1.2f (%1.2f)", mean(x), sd(x))
}))


#拟合gammar分布回归
zs.hs.hz1$心血管资源住院费用1=zs.hs.hz1$心血管资源住院费用+1
ggplot(zs.hs.hz1, aes(log(心血管资源住院费用1), colour = 是否使用丹参)) + geom_density()


m1<-glm(心血管资源住院费用1~.,data=zs.hs.hz1[,-1],family=Gamma(link = 'identity'))
summary(m1)
...
zs.hs.hz1=cb.zs.hs.hz1[,c("心血管资源门诊加住院费用","性别","保险类别","age","是否使用丹参",'一年心血管次数.x','住院天数.x','门诊费用.x','住院费用.x')]

write.table(zs.hs.hz1,file = 'glm1.csv',sep = ',',row.names = F)

#基本情况描述
ggplot(zs.hs.hz1, aes(心血管资源门诊加住院费用, colour = 是否使用丹参)) + geom_density()


with(zs.hs.hz1, tapply(心血管资源门诊加住院费用, 是否使用丹参, function(x) {
    sprintf("M (SD) = %1.2f (%1.2f)", mean(x), sd(x))
}))


#拟合gammar分布回归
zs.hs.hz1$心血管资源门诊加住院费用1=zs.hs.hz1$心血管资源门诊加住院费用+1
ggplot(zs.hs.hz1, aes(log(心血管资源门诊加住院费用1), colour = 是否使用丹参)) + geom_density()


m1<-glm(心血管资源门诊加住院费用1~.,data=zs.hs.hz1[,-1],family=Gamma(link = 'identity'))
summary(m1)
