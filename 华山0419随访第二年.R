...
dz.zyts$twoyear=paste0(substr(dz.zyts$startday,1,3),as.character(as.numeric(substr(dz.zyts$startday,4,4))+2),substr(dz.zyts$startday,5,10))
dz.zyts$twoyear=ifelse(substr(dz.zyts$twoyear,1,4)=='2001',paste0(substr(dz.zyts$startday,1,2),10,substr(dz.zyts$startday,5,10)),dz.zyts$twoyear)
dz.zyts$oneyear=ifelse(substr(dz.zyts$oneyear,9,10)=='29',paste0(substr(dz.zyts$oneyear,1,8),28),dz.zyts$oneyear)
dz.zyts$twoyear=ifelse(substr(dz.zyts$twoyear,9,10)=='29',paste0(substr(dz.zyts$twoyear,1,8),28),dz.zyts$twoyear)

table2.dzmzhz$MAX.E.JZBM.=as.character(table2.dzmzhz$MAX.E.JZBM.)
...
dz.mzts$MAX.KFRQ.=as.Date(as.character(dz.mzts$MAX.KFRQ.))
dz.mzts$twoyear=as.Date(as.character(dz.mzts$twoyear))
dz.mzts$oneyear=as.Date(as.character(dz.mzts$oneyear))
dz.mzts1=dz.mzts%>%
  filter(MAX.KFRQ.>=oneyear&MAX.KFRQ.<=twoyear)
...
#计算发生频率
table2.symzhz1$门诊号=as.character(table2.symzhz1$门诊号)
dz.zyts=dz.zyts%>%
...
#每次的费用
hs.id.dz2=hs.id.dz1[,c("MEDCARD","CURENO","oneyear","twoyear")]
...
table3.dzzzysj1$twoyear=as.Date(as.character(table3.dzzzysj1$twoyear))
...
table3.dzzzysj1=table3.dzzzysj1%>%
  filter(STARTTIME>=oneyear&STARTTIME<=twoyear)
...
table3.dzzzysj2$twoyear=as.Date(as.character(table3.dzzzysj2$twoyear))
...
table3.dzzzysj2=table3.dzzzysj2%>%
  filter(LASTCREDITTIME>=oneyear&LASTCREDITTIME<=twoyear)
...
hs.id.dz.heart2=hs.id.dz.heart1[,c("MEDCARD","CURENO","twoyear","oneyear")]
...
table2.dzmzmx1$twoyear=as.Date(as.character(table2.dzmzmx1$twoyear))
table2.dzmzmx1$oneyear=as.Date(as.character(table2.dzmzmx1$oneyear))
table2.dzmzmx1$KFRQ=as.Date(as.character(table2.dzmzmx1$KFRQ))
table2.dzmzmx1=table2.dzmzmx1%>%
  filter(KFRQ>=oneyear&KFRQ<=twoyear)

hs.id.dz3$门诊费用1=rep(0,nrow(hs.id.dz3))
for (i in 1:nrow(hs.id.dz3)) {
...
table3.dzzmzsj1$twoyear=as.Date(as.character(table3.dzzmzsj1$twoyear))
table3.dzzmzsj1$oneyear=as.Date(as.character(table3.dzzmzsj1$oneyear))
table3.dzzmzsj1$CZRQ=as.Date(as.character(table3.dzzmzsj1$CZRQ))
table3.dzzmzsj1=table3.dzzmzsj1%>%
  filter(CZRQ>=oneyear&CZRQ<=twoyear)

hs.id.dz3$门诊费用2=rep(0,nrow(hs.id.dz3))
for (i in 1:nrow(hs.id.dz3)) {
...
dz.mz.heart$MAX.KFRQ.=as.Date(as.character(dz.mz.heart$MAX.KFRQ.))
dz.mz.heart$twoyear=as.Date(as.character(dz.mz.heart$twoyear))
dz.mz.heart$oneyear=as.Date(as.character(dz.mz.heart$oneyear))
...
dz.mz.heart2=dz.mz.heart2[,c("门诊号","twoyear","oneyear","MZLSH")]
...
write.table(hs.id.dz3,file = "华山对照组0419随访2年.csv",sep=',',row.names=F)
...
table2.case.id$twoyear<-paste0(substr(table2.case.id[,2],1,3),as.character(as.numeric(substr(table2.case.id[,2],4,4))+2),substr(table2.case.id[,2],5,19))
table2.case.id$twoyear<-ifelse(substr(table2.case.id$twoyear,1,4)=='2001',paste0(substr(table2.case.id$twoyear,1,2),10,substr(table2.case.id$twoyear,5,19)),table2.case.id$twoyear)
...
table2.syzyhz1$oneyear=as.Date(table2.syzyhz1$oneyear)
table2.syzyhz1$twoyear=as.Date(table2.syzyhz1$twoyear)
table2.syzyhz1$ENTDATE=as.Date(as.character(table2.syzyhz1$ENTDATE))
table2.syzyhz2=table2.syzyhz1%>%
  filter(ENTDATE>=oneyear&ENTDATE<=twoyear)                #随访一年住院就诊记录
...
table3.syzmzsj$KHMZH=as.character(table3.syzmzsj$KHMZH)

table2.case.id1$oneyear=as.Date(table2.case.id1$oneyear)
table2.case.id1$twoyear=as.Date(table2.case.id1$twoyear)
table3.syzmzsj$CZRQ=as.Date(as.character(table3.syzmzsj$CZRQ))

table3.syzmzsj1=table2.case.id1%>%
  left_join(table3.syzmzsj%>%
              rename(ID=KHMZH),by="ID")%>%
  filter(CZRQ>=oneyear&CZRQ<=twoyear)
...
table3.syzzysj$LASTCREDITTIME=as.Date(as.character(table3.syzzysj$LASTCREDITTIME))
table2.case.id1$oneyear=as.Date(table2.case.id1$oneyear)
table2.case.id1$twoyear=as.Date(table2.case.id1$twoyear)
...
write.table(table2.case.id1,file = "华山实验组0419随访2年结果.csv",sep=',',row.names=F)
